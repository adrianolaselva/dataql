...
import (
	...
	yamlExport "github.com/adrianolaselva/dataql/pkg/exportdata/yaml"
...
func TestYamlExport_Export_Success(t *testing.T) {
	tmpDir := t.TempDir()
	...
	exportPath := filepath.Join(tmpDir, "output.yaml")
	bar := createProgressBar()

	exporter := yamlExport.NewYamlExport(rows, exportPath, bar)
...
func TestYamlExport_Export_EmptyResult(t *testing.T) {
	tmpDir := t.TempDir()
...
func TestYamlExport_Export_SpecialCharacters(t *testing.T) {
	tmpDir := t.TempDir()
	...
	err = storage.BuildStructure("test_table", []string{"id", "name", "value"})
	require.NoError(t, err)
	err = storage.InsertRow("test_table", []string{"id", "name", "value"}, []any{"1", "a,b", "<x>: special # chars"})
	require.NoError(t, err)

	rows, err := storage.Query("SELECT * FROM test_table")
	require.NoError(t, err)

	exportPath := filepath.Join(tmpDir, "special.yaml")
	bar := createProgressBar()
	...
	assert.Equal(t, "<x>: special # chars", toString(records[0]["value"]))
}
