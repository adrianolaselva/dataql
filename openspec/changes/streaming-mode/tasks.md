...
- [x] 2.1 `--window <count|duration>` + `--interval <duration>`: maintain a bounded `stream` table in DuckDB (append + evict by count or by an `_ingested_at` time column).
- [x] 2.2 Interval ticker runs `-q` over the window and emits the result; final emit on termination.
- [x] 2.3 Unit tests for windowing (count eviction, duration eviction, interval emit) + a windowed Kafka E2E (rolling COUNT).

## 3. Docs & wrap-up

- [x] 3.1 Document `--follow` (emit + window, termination) in `docs/data-sources.md` and the README; add an MCP/agent note (streaming sampling).
- [x] 3.2 Keep the coverage ratchet green (StreamRunner well-tested); bump baseline.
