...
# Comprehensive tests for MySQL connectivity and query functionality
#
# Test Coverage:
...
test_select_specific_columns() {
    ...
    result=$($DATAQL_BIN run -q "SELECT name, email FROM test_data" -f "$DATAQL_TEST_MYSQL_URL" 2>&1)
...
test_select_with_alias() {
    ...
    result=$($DATAQL_BIN run -q "SELECT name AS user_name FROM test_data" -f "$DATAQL_TEST_MYSQL_URL" 2>&1)
...
test_where_equals() {
    ...
    result=$($DATAQL_BIN run -q "SELECT * FROM test_data WHERE name = 'Alice'" -f "$DATAQL_TEST_MYSQL_URL" 2>&1)
    if echo "$result" | grep -q "Alice" && ! echo "$result" | grep -q "Bob"; then
...
test_where_greater_than() {
    ...
    result=$($DATAQL_BIN run -q "SELECT name, age FROM test_data WHERE age > 30" -f "$DATAQL_TEST_MYSQL_URL" 2>&1)
...
test_where_and() {
    ...
    result=$($DATAQL_BIN run -q "SELECT name FROM test_data WHERE age > 25 AND age < 40" -f "$DATAQL_TEST_MYSQL_URL" 2>&1)
...
test_where_or() {
    ...
    result=$($DATAQL_BIN run -q "SELECT name FROM test_data WHERE name = 'Alice' OR name = 'Eve'" -f "$DATAQL_TEST_MYSQL_URL" 2>&1)
    if echo "$result" | grep -q "Alice" && echo "$result" | grep -q "Eve"; then
...
test_where_in() {
    ...
    result=$($DATAQL_BIN run -q "SELECT name FROM test_data WHERE name IN ('Alice', 'Bob')" -f "$DATAQL_TEST_MYSQL_URL" 2>&1)
...
test_where_like() {
    ...
    result=$($DATAQL_BIN run -q "SELECT email FROM test_data WHERE email LIKE '%@example.com'" -f "$DATAQL_TEST_MYSQL_URL" 2>&1)
...
test_where_between() {
    ...
    result=$($DATAQL_BIN run -q "SELECT name, age FROM test_data WHERE age BETWEEN 25 AND 35" -f "$DATAQL_TEST_MYSQL_URL" 2>&1)
...
test_order_by_asc() {
    ...
    result=$($DATAQL_BIN run -q "SELECT name, age FROM test_data ORDER BY age ASC" -f "$DATAQL_TEST_MYSQL_URL" 2>&1)
...
test_order_by_desc() {
    ...
    result=$($DATAQL_BIN run -q "SELECT name, age FROM test_data ORDER BY age DESC" -f "$DATAQL_TEST_MYSQL_URL" 2>&1)
...
test_limit_offset() {
    ...
    result=$($DATAQL_BIN run -q "SELECT name FROM test_data ORDER BY id LIMIT 2 OFFSET 1" -f "$DATAQL_TEST_MYSQL_URL" 2>&1)
...
test_count_with_where() {
    ...
    result=$($DATAQL_BIN run -q "SELECT COUNT(*) AS count FROM test_data WHERE age > 30" -f "$DATAQL_TEST_MYSQL_URL" 2>&1)
...
test_sum() {
    ...
    result=$($DATAQL_BIN run -q "SELECT SUM(age) AS total_age FROM test_data" -f "$DATAQL_TEST_MYSQL_URL" 2>&1)
...
test_avg() {
    ...
    result=$($DATAQL_BIN run -q "SELECT AVG(age) AS avg_age FROM test_data" -f "$DATAQL_TEST_MYSQL_URL" 2>&1)
...
test_min() {
    ...
    result=$($DATAQL_BIN run -q "SELECT MIN(age) AS min_age FROM test_data" -f "$DATAQL_TEST_MYSQL_URL" 2>&1)
...
test_max() {
    ...
    result=$($DATAQL_BIN run -q "SELECT MAX(age) AS max_age FROM test_data" -f "$DATAQL_TEST_MYSQL_URL" 2>&1)
...
test_export_jsonl() {
    ...
    output_file="/tmp/mysql_export_$$.jsonl"
    $DATAQL_BIN run -q "SELECT name, email FROM test_data LIMIT 2" -f "$DATAQL_TEST_MYSQL_URL" -e "$output_file" -t jsonl 2>&1 > /dev/null
...
test_export_json() {
    ...
    output_file="/tmp/mysql_export_$$.json"
    $DATAQL_BIN run -q "SELECT name, email FROM test_data LIMIT 2" -f "$DATAQL_TEST_MYSQL_URL" -e "$output_file" -t json 2>&1 > /dev/null
    if [ -f "$output_file" ] && grep -q "Alice" "$output_file"; then
...
test_query_users_table() {
    ...
    users_url="mysql://dataql:dataql_pass@localhost:23306/dataql_test/users"
...
test_query_departments_table() {
    ...
    dept_url="mysql://dataql:dataql_pass@localhost:23306/dataql_test/departments"
